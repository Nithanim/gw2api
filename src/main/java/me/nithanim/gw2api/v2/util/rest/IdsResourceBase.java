 ...
 * @param <OVERVIEW_CLASS> The class of the object that is returned when querying all possible ids
 */
public class IdsResourceBase<DATA_CLASS, OVERVIEW_CLASS> {
  ...
  public IdsResourceBase(
      WebTarget webTarget, Class<DATA_CLASS> dataClass, Class<OVERVIEW_CLASS> overviewClass) {
  ...
  public DATA_CLASS get(int id, String language) {
    return RequestHelper.INSTANCE.getRequest(
        webTarget.path(String.valueOf(id)), dataClass, "lang", language);
  }
  ...
  public DATA_CLASS[] getAll(String language) {
    return RequestHelper.INSTANCE.getRequest(
        webTarget, dataClassArray, "ids", "all", "lang", language);
  }
}
